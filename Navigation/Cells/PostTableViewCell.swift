...
final class PostTableViewCell: UITableViewCell {
    ...
    private func setupView() {
        ...
        self.backView.addSubview(self.author)
        ...
        let backConstraints = self.backViewConstraints()
        let authorConstraints = self.authorLabelConstraints()
        let imageConstraints = self.imageConstraints()
        let descriptionConstraints = self.descriptionLabelConstraints()
    ...
    private func authorLabelConstraints() -> [NSLayoutConstraint] {
        let topConstraint = self.author.topAnchor.constraint(equalTo: self.backView.topAnchor, constant: 16)
        let leadingConstraint = self.author.leadingAnchor.constraint(equalTo: self.backView.leadingAnchor, constant: 16)
        let trailingConstraint = self.author.trailingAnchor.constraint(equalTo: self.backView.trailingAnchor, constant: -16)

        return [ topConstraint, leadingConstraint, trailingConstraint ]
    }

    private func imageConstraints() -> [NSLayoutConstraint] {
        let topConstraint = self.postImageView.topAnchor.constraint(equalTo: self.author.bottomAnchor, constant: 12)
...
extension PostTableViewCell: Setupable {
    ...
    func setup(with viewModel: ViewModelProtocol) {
        ...
        self.likes.text? += String(viewModel.likes)
        self.views.text? += String(viewModel.views)
    }
}
